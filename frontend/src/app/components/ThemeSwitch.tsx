...
export default function ThemeSwitch (): JSX.Element {
  ...
  const nextTheme = useCallback( () => { setthemeId( () => ( (themeId + 1)%themesCount ) ) }, [themeId, themesCount] );
  const switchTheme = useCallback(() => { nextTheme()}, [nextTheme]);
  ...
  return (
    <>
      <label className="btn btn-ghost btn-circle swap swap-rotate scale-75 hover:scale-90 transition-all duration-300 ease-in-out">
        ...
        <input type="checkbox" checked={swapDefaultChecked} onClick={switchTheme}
